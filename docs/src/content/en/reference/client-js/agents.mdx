...
When `@mastra/observability` is installed and configured on the server, a client-side tool records a `CLIENT_TOOL_CALL` event span as a child of the current `AGENT_RUN` span. Event spans occur at a point in time (no end time) and mark "this client tool was deferred to the client at this moment in the trace." Without server-side observability configured, client tool tracing is a no-op.
