...
enum SpanType {
   ...
   * server (occurs at a point in time, no endTime). Child spans/logs
   * from the client SDK flow back as OTLP/JSON via the
   * ClientToolObservabilityIngest interface in @mastra/observability
   * and parent themselves under this event span. See the "Client
   * tools" section in the @mastra/client-js reference for the full
   * flow.
   */
